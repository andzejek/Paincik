...
int CEditor::onMouseMove(short x,short y,int keys){
    ...
    if(selecting){
        if(changeLeftSelectRectBorder)      selectStart.x=x;
        if(changeRightSelectRectBorder)     selectEnd.x=x;
        if(changeTopSelectRectBorder)       selectStart.y=y;
        if(changeBottomSelectRectBorder)    selectEnd.y=y;
    ...
    if(lButtonHold){
                ...
                switch(paintMode){
                    ...
                    case MODE_SPRAY: {
                        ...
                        CSprayFast fastSpray(curr,penSize,cover,color1,((uint64_t)hi << 32) | lo,paintBuff->width,paintBuff->height);
